...
impl<'a> MetricsAggregator<'a> {
    ...
    fn compute_by_cohort(transactions: &[TransactionMetrics]) -> Vec<CohortMetrics> {
        ...
        let labels = [
            SubmitCohortLabel::Plain,
            SubmitCohortLabel::ValidityPass,
            SubmitCohortLabel::ValidityControl,
        ];
        labels
            .into_iter()
            .filter_map(|cohort| {
...
mod tests {
    ...
    fn by_cohort_splits_confirmed_across_cohorts() {
        let transactions = [
            ...
            cohort_transaction(13, 400, SubmitCohortLabel::ValidityControl),
        ];

        let by_cohort = MetricsAggregator::compute_by_cohort(&transactions);

        assert_eq!(by_cohort.len(), 3);
        ...
        assert_eq!(by_cohort[2].cohort, SubmitCohortLabel::ValidityControl);
        assert_eq!(by_cohort[2].confirmed, 1);
        assert_eq!(by_cohort[2].total_gas, 400);
    }
}
