...
/// Salt mixed into the per-sender empty-predicate-control routing hash.
const VALIDITY_CONTROL_SALT: u64 = 0x7661_6c5f_6374_726c; // "val_ctrl"
...
impl ValidityRouter {
    ...
    pub fn new(config: &LoadConfig) -> Self {
        Self {
            ...
            control_enabled: config.validity_empty_control,
            control_ratio: config.validity_control_ratio,
    ...
    pub fn cohort_for_sender(&self, sender: Address) -> SubmitCohort {
        ...
        if self.control_enabled
            && sender_in_fraction(sender, self.seed, VALIDITY_CONTROL_SALT, self.control_ratio)
        {
            return SubmitCohort::ValidityControl;
        }
        SubmitCohort::ValidityPass
    }

    /// Resolves the configured predicate templates into concrete predicates for
    /// a transaction. Returns an empty vector for cohorts that carry no
    /// predicates (plain and control).
    pub fn predicates_for(
        ...
        match cohort {
            ...
            SubmitCohort::Plain | SubmitCohort::ValidityControl => Vec::new(),
        }
    }
...
mod tests {
    ...
    #[test]
    fn control_cohort_splits_validity_senders() {
        let r = ValidityRouter {
            ratio: 1.0,
        ...
        for i in 0..total {
            let mut bytes = [0u8; 20];
            bytes[..4].copy_from_slice(&i.to_be_bytes());
            match r.cohort_for_sender(Address::from(bytes)) {
                SubmitCohort::ValidityControl => control += 1,
                SubmitCohort::ValidityPass => pass += 1,
                other => panic!("unexpected cohort {other:?}"),
            }
        }
        let control_fraction = control as f64 / (control + pass) as f64;
        assert!(
            (control_fraction - 0.5).abs() < 0.05,
            "control fraction {control_fraction} should be near 0.5"
        );
    }
    ...
    #[test]
    fn control_and_plain_cohorts_carry_no_predicates() {
        ...
        assert!(r.predicates_for(SubmitCohort::ValidityControl, from, None).is_empty());
    }
    ...
    #[test]
    fn mapping_balance_of_slot_matches_solidity_layout() {
        ...
        let templates = vec![ValidityPredicateTemplate::Storage {
            ...
            slot: SlotTemplate::MappingBalanceOf {
