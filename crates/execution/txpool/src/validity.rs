...
impl ValidatedTransactionExtensions<BasePooledTransaction> for TransactionValidity {
    ...
    /// `base_sendRawTransactionValidity`. It re-checks the count bound and each
    /// predicate's parameters as defense-in-depth against a misbehaving upstream,
    /// but does not reject an empty predicate set: the builder legitimately
    /// receives ordinary transactions that carry no predicates. The stronger
    /// "non-empty" rule belongs only to the advanced-transaction submission
    /// endpoint on mempool nodes.
