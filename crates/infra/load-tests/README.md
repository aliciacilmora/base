...
fixed `0x` address. Storage slots are either a `fixed` slot or a
`mapping_balance_of` slot, which computes the Solidity mapping slot
`keccak256(key ++ mapping_slot)` so `balanceOf(key)` slots are expressible.
Values, slots, and masks accept hex (`0x...`) or decimal strings. At most 64
predicates may be attached per transaction.

**Empty-predicate control cohort.** Validity transactions ingress with a
different origin (private + forwarding) than plain transactions, so a raw
plain-vs-validity comparison conflates transport cost with predicate-evaluation
cost. Enabling `empty_predicate_control` routes `control_ratio` of the validity
senders through the same endpoint but with an empty predicate list. The final
summary's `by_cohort` breakdown then lets you decompose cost: `plain` is the
baseline, `control − plain` is the origin/forwarding overhead, and
`pass − control` is the predicate-evaluation cost.
