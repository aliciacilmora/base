...
/// Estimated Span channel boundaries must preserve the sequencer's exact chain.
#[tokio::test]
async fn batcher_span_size_boundaries_derive_exact_blocks() {
    ...
    Batcher::new(source, &h.rollup_config, base_config).advance(&mut h.l1).await;
    chain.push(h.l1.tip().clone());

    node.initialize().await;
    let derived = node.run_until_idle().await;
    assert_eq!(derived, 4, "all blocks across estimated channel boundaries must derive");
